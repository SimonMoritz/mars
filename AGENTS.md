...
- **worker/** — Cloudflare Worker that serves the static frontend, `/api/photos` (full ID list), and `/api/photo/:id` (one photo's metadata). Images are served from R2 via `/img/*`, metadata is read from KV.
...
- `worker/public/js/ui.js` expects photo metadata fields named `title`, `query`, `center`, `date`, `nasaId`, and `imageUrl`. If the scraper metadata shape changes, update the UI and `/api/photo/:id` contract together.
...
- `worker/src/index.js` ends with `return env.ASSETS.fetch(request)` so the asset binding handles anything not matched by an explicit route (and returns its own 404 for genuine misses). Don't return `undefined` — with the `ASSETS` binding configured, the runtime now requires every reachable path to resolve to a `Response`.
- The `/photo/:id` route HTMLRewrites the `/` shell, targeting these selectors: `title`, `link[rel="canonical"]`, `meta[name="description"]`, `meta[property="og:url|og:title|og:description|og:image"]`, `meta[name="twitter:title|twitter:description|twitter:image"]`, and `img#photo`. If you rename or restructure those in `worker/public/index.html`, update `handlePhotoPage` in lockstep — mismatches fail silently (the page renders without the SEO content).
- Asset references in `worker/public/index.html` must be root-relative (`/styles.css`, `/js/main.js`, `/favicon-framesfrommars.svg`). Relative paths break under `/photo/:id` because they resolve against the current pathname.
- HTMLRewriter handler objects must not carry property names that collide with handler keys (`element`, `text`, `comments`). A class with a `text` property breaks at runtime because HTMLRewriter sees the property and tries to invoke it as the text-node handler. Prefer factory functions returning plain `{ element() {} }` objects over class instances.
